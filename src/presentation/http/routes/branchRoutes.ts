...
import { branchIdSchema, branchInventorySharingSchema, branchMembersSchema, branchStatusSchema, createBranchSchema, updateBranchSchema } from '../schemas/branchSchemas.js';
...
export async function branchRoutes(app: FastifyInstance) {
  ...
  app.get('/branches/:id/inventory-sharing', { preHandler: [app.authenticate, app.authorize('branches', 'read')] }, async (request) => { const { id } = branchIdSchema.parse(request.params); return { data: await branches.inventorySharing(request.authUser!, id) }; });
  app.put('/branches/:id/inventory-sharing', { preHandler: [app.authenticate, app.authorize('branches', 'update')] }, async (request) => { const { id } = branchIdSchema.parse(request.params); return { data: await branches.configureInventorySharing(request.authUser!, id, branchInventorySharingSchema.parse(request.body)) }; });
}
