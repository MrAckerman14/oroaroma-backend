...
const expenseControlObjectSchema = z.object({
  amount: amountSchema,
...
export const createExpenseControlSchema = expenseControlObjectSchema.refine((value) => Boolean(value.categoryId) !== Boolean(value.categoryName), {
  message: 'Use categoryId o categoryName, pero no ambos',
  path: ['categoryId']
});

export const updateExpenseControlSchema = expenseControlObjectSchema.partial().refine(
